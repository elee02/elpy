...
bool Rectangle::IsInside(const Point& pt) const {
    int x_min = m_lowLeft.GetX();
    int x_max = m_upRight.GetX();
    int y_min = m_lowLeft.GetY();
    int y_max = m_upRight.GetY();
    bool x_isIn = (pt.GetX() >= x_min) && (pt.GetX() <= x_max);
    bool y_isIn = (pt.GetY() >= y_min) && (pt.GetY() <= y_max);
    if (x_isIn && y_isIn)
        return true;
    return false;
}
